...
function sanitizeMacMetadata() {
  ...
  const targets = ["assets", "dist", "src", "index.html", "release"]
  ...
  if (process.platform === "darwin" && code === 0 && !hasCustomOutput && fs.existsSync(tempOutputDir)) {
    try {
      ...
      for (const entry of entries) {
        ...
        // Remove the previous artifact first so macOS framework symlinks can
        // replace older directories cleanly on repeat builds.
